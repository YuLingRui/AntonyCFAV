...
public class ImageLoader {
    ...
    private DiskLruCache mDiskLruCache;//设备缓存
    private boolean mIsDiskLruCacheCreated; //标记是否已经开启了磁盘缓存
    ...
    private ImageLoader(Context context) {
        ...
        //判断磁盘路径下的可用空间， 是否达到预期大小， 如果达到，那就创建磁盘缓存对象
        if (getUsableSpace(diskCacheDir) > DISK_CACHE_SIZE) {
            try {
                mDiskLruCache = DiskLruCache.open(diskCacheDir, 1, 1, DISK_CACHE_SIZE);
                mIsDiskLruCacheCreated = true;
                Log.i(TAG, "设置--设备磁盘缓存成功--> 路径为:" + diskCacheDir.getPath());
            } catch (IOException e) {
                e.printStackTrace();
            }
    ...
    public File getDiskCacheDir(Context context, String dirName) {
        ...
        //判断SD卡是否挂起
        boolean externallsAlive = Environment.getExternalStorageState().equals(Environment.MEDIA_MOUNTED);
        if (externallsAlive) {
            File externalCacheDir = context.getExternalCacheDir();
            if (externalCacheDir == null) {
                //API=29 弃用了getExternalStorageDirectory()方法
                cachePath = Environment.getExternalStorageDirectory().getPath();
                Log.i(TAG, "path1: " + cachePath);
            } else {
                cachePath = externalCacheDir.getPath();
                Log.i(TAG, "path2: " + cachePath);
            }
        } else {
            cachePath = context.getCacheDir().getPath();
            Log.i(TAG, "path3: " + cachePath);
        }
    ...
    private Bitmap loadBitmapFromDiskCache(String url, int reqWidth, int reqHeight) throws IOException {
        ...
        if (null != snapshot) {
            ...
            bitmap = ImageResizer.decodeSampledBitmapFormFileDescriptor(fd, reqWidth, reqHeight);
